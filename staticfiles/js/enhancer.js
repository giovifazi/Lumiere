var BooksearchApi = (function() {
   ...
   var API_URL = "https://www.googleapis.com/books/v1/volumes?q="
      ...
      wqa.queryBooks = function(titles) {
         $.ajax({
            url: API_URL + titles,
            dataType: "json",

            success: function(data) {
               herebooks.innerHTML += "<tr> <th>Title</th> <th>Date</th> <th>Link</th> </tr>"

               for(i=0; i<data.items.length; i++){
                  herebooks.innerHTML += '<tr> <th>' + data.items[i].volumeInfo.title + '</th> <th>' + data.items[i].volumeInfo.publishedDate + '</th> <th><a href="' + data.items[i].volumeInfo.previewLink + '">Click here</a></th></tr>'
               }
            },
            ...
            error: function(xhr, result, status){
               herebooks.innerHTML += "<p>Could not find any books</p>"
            }
         });
      };
...
var WikiquoteApi = (function() {
  ...
  // Simple cleanup func
  wqa.cleanupStrings = function(arr) {
     var newArr = [];
  ...
  wqa.randq = function(qsearch) {
     WikiquoteApi.getRandomQuote(qsearch, function(newQuote){
        q = wqa.cleanupStrings(newQuote.quote.split('.'))
           document.getElementById("herequote").innerHTML = q; 
        document.getElementById("tweetquote").href = "https://twitter.com/intent/tweet?text=" + q;
     }, 
     function(){
        var elem = document.getElementById("noquote");
